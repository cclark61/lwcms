...
if ($do_trans && $ssv_status) {
	...
	switch ($action) {
	    case "insert":
	        ...
	        $change_id = $sbc->save();
	        add_action_message("The folder category has been successfully saved.");
	        break;
	    ...
	    case "delete":
	    	if (isset($button_1) && $button_1 == "Delete") {
	    		if (isset($used_content_cats[$id])) {
	        		...
	        		$sbc->delete($id);
	        		add_action_message("The folder category has been successfully deleted.");
				}
        	}
        	else {
        		add_warn_message("The folder category was not deleted.");
        	}
        	break;
	}
}
